...
#Modified to get TEST indices.

# pip
import numpy as np

def Normalize(data, ix_scalar, minis=[], maxis=[]):
    '''
    ...
    ix_directional - directional columns indexes    #REMOVED SINCE IT DOESN'T NEED DIRECTIONAL VARIABLES.
    '''
...
def DeNormalize(data_norm, ix_scalar, minis, maxis):
    '''
...
def Normalized_Distance(M, D, ix_scalar):
    '''
...
def MaxDiss_Simplified_NoThreshold(data, num_centers, ix_scalar):
    '''
    ...
    # normalize scalar and directional data
    data_norm, minis, maxis = Normalize(data, ix_scalar)
    ...
    while n_c < num_centers:
        ...
        if m2 == 1:
            ...
            d_last = Normalized_Distance(train, xx2, ix_scalar)

        else:
            xx = np.array([subset[-1,:]])
            xx2 = np.repeat(xx, train.shape[0], axis=0)
            d_prev = Normalized_Distance(train, xx2, ix_scalar)
    ...
    # normalize scalar and directional data
    centroids = DeNormalize(subset, ix_scalar, minis, maxis) #TRAIN DATA
    test = DeNormalize(train, ix_scalar, minis, maxis) #TEST DATA
